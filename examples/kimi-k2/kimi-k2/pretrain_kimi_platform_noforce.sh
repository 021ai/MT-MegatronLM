...
WORK_HOME=/mnt/moer-train/public/suzy/baseline/megatron-lm-musa-patch/examples/kimi-k2
PATCH_HOME=/mnt/moer-train/public/suzy/baseline/megatron-lm-musa-patch
TP_SIZE=${TP:-1}
PP_SIZE=${PP:-31}
EP_SIZE=${EP:-8}
FP8=${FP8:-false}
...
EXPNAME="tp${TP_SIZE}_pp${PP_SIZE}_dp${DP_SIZE}_mbs${MICRO_BATCH_SIZE}_numbs${NUM_MICROBATCHES}_gbs${GLOBAL_BATCH_SIZE}_gpus${WORLD_SIZE}_mtp${MTP}_forcelb${FORCE_LB}_pertensor${FP8}"

# profiling setting
export ENABLE_PROFILER=0
export PROFILER_FREQ=250
export PROFILER_SAVE_DIR=/mnt/moer-train/public/kimi_16layer/trace/noforce/
...
CHECKPOINT_PATH=/mnt/moer-train/public/suzy/checkpoints/test_model
# CHECKPOINT_PATH=/mnt/moer-train/public/kimi_data_bf16/checkpoints/${EXPNAME}_load_from_iter_1600_triton_deepep-0126-zj
# CHECKPOINT_PATH=/mnt/moer-train/public/kimi_data_bf16/checkpoints/tp1_pp31_dp_mbs1_numbs_gbs4096_gpus0_mtp0_forcelbfalse_pertensorfalse_NO_LOSS_REDUCE_load_from_iter_1600_triton_deepep-0126-zj
...
RUN_DIR=${OUTPUT_DIR}/kimi-1000B-128t-bf16-noforce/${CURRENT_TIME}/
mkdir -p "${RUN_DIR}"
LOG_FILE_TMP="${RUN_DIR}/${EXPNAME}.RANK${NODE_RANK}.${NODE_ADDR}.log"
LOG_FILE=${LOG_FILE:-${LOG_FILE_TMP}}

# model configs
if [ $MODEL_SIZE = large ]; then     # large models, 1000B official
LAYERS=${LAYERS:-61}
MODEL_ARGS=(
...
TRAINING_ARGS=(
    ...
    # --recompute-modules mla_up_proj moe layernorm mlp
    # ==================================================================
    --manual-gc
    ...
    --memory-snapshot-path /mnt/moer-train/public/suzy/baseline/megatron-lm-musa-patch/examples/kimi-k2/mem_snapshot/2026-02-09_1127/snapshot.pickle
)
...
TOKEN_DISPATCHER=${TOKEN_DISPATCHER:-alltoall}
